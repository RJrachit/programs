...
{
temp[i] = temp[i] - qt; total = total + qt;
}
if(temp[i] == 0 && counter == 1)
{
x--;
printf("\nProcess%d\t\t\t%d\t\t%d\t\t %d\t\t\t %d", i + 1,bt[i],at[i],total - at[i], total - at[i] - bt[i]);
wt = wt + total - at[i] - bt[i]; tt = tt + total - at[i];  counter = 0;
}
if(i == n - 1)
{
i = 0;
...
average_wait_time = wt * 1.0 / n; average_turnaround_time = tt * 1.0 / n;
